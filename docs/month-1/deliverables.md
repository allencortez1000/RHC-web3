...
- PostgreSQL domain schema and **five pending migrations**, including identity/history/ledger reconciliation, application PostgREST lockdown, and reservation/property-status history foundation. Migration source is not deployed protection.
...
- API-backed customer profile, property relationships, reservation foundation, directory, notifications and append-only consent history; explicit loading/empty/error/disabled states rather than successful fixture fallback.
