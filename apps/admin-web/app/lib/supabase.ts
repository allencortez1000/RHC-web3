...
function readDemoSession() {
  ...
  const email = decodeURIComponent(cookieEmail).toLowerCase();
  return adminEmails.has(email) ? demoSessionFor(email) : null;
}
...
export const authAdapter: AuthAdapter = {
  ...
  async login(email, password) {
    ...
    if (adminEmails.has(normalized) && password === demoPassword) {
