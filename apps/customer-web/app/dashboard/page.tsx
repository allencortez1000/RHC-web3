    ...
    <AppShell title="RHC Web3 Dashboard" navItems={navFor('Dashboard')}>
      <section id="dashboard" className="grid gap-5 xl:grid-cols-[1.15fr_.85fr]">
        <div className="rhc-card rhc-card-token border bg-[var(--rhc-surface)] p-6 shadow-sm md:p-7">
          ...
          <h2 className="rhc-page-title mt-5 max-w-3xl">Real assets. Digital ownership. Connected through Web3.</h2>
          <p className="rhc-body-copy mt-4 max-w-3xl">A premium digital ecosystem for RHC Token-inspired services, digital identity, wallets, property records, marketplace activity, RHC Points, and future Web3 assets. Local preview uses demo data only.</p>
          <div className="mt-7 flex flex-wrap gap-3">
            <Web3Button href="/token">View RHC Token</Web3Button>
            <Web3Button href="#wallet" variant="secondary">Open Wallet Summary</Web3Button>
      ...
      <section className="mt-5"><TokenHero /></section>

      <section className="mt-5 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="RHC Token Supply" value="Not live" detail="Awaiting blockchain deployment" icon="RHC" />
        <MetricCard label="RHC Points" value="12,850" detail="Demo rewards balance" icon="PT" trend="+850" />
        <MetricCard label="Digital Properties" value="3" detail="Development property records" icon="PR" />
        <MetricCard label="Wallet Status" value="Demo" detail="No custody or real transfer active" icon="WL" />
      </section>
        ...
        <div className="grid gap-6">
          <Card id="wallet" title="RHC Wallet">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[var(--rhc-muted)]">Total Portfolio Value</p>
      ...
      <section id="points" className="mt-5 grid gap-5 xl:grid-cols-[.9fr_1.1fr]">
        <Card title="RHC Points">
          <p className="text-sm leading-6 text-[var(--rhc-muted)]">A local rewards ledger for RHC services and customer activity. RHC Points are separate from the future RHC Token and are shown here as demo rewards data.</p>
        ...
        <Card title="Token Information">
          <div className="grid gap-3 sm:grid-cols-2">
            {[
              ['Reward Name', 'RHC Points'], ['Symbol', 'RHC Points'], ['Network', 'Off-chain rewards ledger'], ['Contract Address', 'Not a token contract'], ['Total Supply', 'Development data'], ['Status', 'Feature flagged']
            ].map(([label, value]) => <div key={label} className="rounded-lg border border-[var(--rhc-border)] bg-[var(--rhc-surface-secondary)] p-4"><p className="text-sm text-[var(--rhc-muted)]">{label}</p><p className="mt-1 font-semibold text-[var(--rhc-heading)]">{value}</p></div>)}
          </div>
        </Card>
      </section>
